...
class HVACConfig:
    ...
    # Calculation switches controlled from the Streamlit Parameter Switches tab.
    # They preserve the same model framework while allowing selected terms to be neutralized.
...
def apply_hvac_preset(cfg: HVACConfig) -> HVACConfig:
    ...
    if (not getattr(cfg, "USE_HVAC_PRESET", True)) or preset is None:
        return out
...
def simulate_baseline_no_degradation(
    ...
    for d in range(cfg.years * cfg.days_per_year):
        ...
        P_fan = ((derived["Q_air_nom_m3h"] * af / 3600.0 * cfg.DP_CLEAN / max(cfg.FAN_EFF, 1e-6)) / 1000.0) if getattr(cfg, "USE_HVAC_FANS", True) else 0.0
...
def cooling_heating_loads(bldg: BuildingSpec, cfg: HVACConfig, derived: Dict[str, float], T_mean: float, RH_mean: float, GHI_mean: float, T_sp: float, occ: float, doy: int) -> Dict[str, float]:
    ...
    n_people = derived["N_people_max"] * occ if getattr(cfg, "USE_PEOPLE_GAINS", True) else 0.0
    internal_kw = derived["Internal_kw_max"] * max(0.20, occ * cfg.INTERNAL_USE_FACTOR + 0.20) if getattr(cfg, "USE_INTERNAL_GAINS", True) else 0.0
    ...
    q_cool_env = cfg.A_COOL_ENV * q_cool_des * (dT_cool / cfg.DT_REF_COOL) if getattr(cfg, "USE_ENVELOPE", True) else 0.0
    q_cool_solar = cfg.SOLAR_COOL_FACTOR * q_cool_des * ghi_norm * max(math.sin(math.pi * doy / 365.0), 0.0) if getattr(cfg, "USE_SOLAR", True) else 0.0
    q_cool_occ = n_people * bldg.sensible_w_per_person / 1000.0 if getattr(cfg, "USE_PEOPLE_GAINS", True) else 0.0
    q_cool_inf = cfg.INFIL_COOL_FACTOR * q_cool_des * (dT_cool / cfg.DT_REF_COOL) if getattr(cfg, "USE_INFILTRATION", True) else 0.0
    q_cool = max(0.0, min((q_cool_env + q_cool_solar + internal_kw + q_cool_occ + q_cool_inf) * humidity_mult, 1.20 * q_cool_des)) if getattr(cfg, "USE_COOLING", True) else 0.0
    q_heat_env = cfg.A_HEAT_ENV * q_heat_des * (dT_heat / cfg.DT_REF_HEAT) if getattr(cfg, "USE_ENVELOPE", True) else 0.0
    q_heat_inf = cfg.INFIL_HEAT_FACTOR * q_heat_des * (dT_heat / cfg.DT_REF_HEAT) if getattr(cfg, "USE_INFILTRATION", True) else 0.0
    q_internal_credit = cfg.HEAT_INTERNAL_CREDIT * (internal_kw + q_cool_occ) if getattr(cfg, "USE_INTERNAL_GAINS", True) else 0.0
    q_heat = max(0.0, min(q_heat_env + q_heat_inf - q_internal_credit, 1.20 * q_heat_des)) if getattr(cfg, "USE_HEATING", True) else 0.0
    if q_cool <= 0.0 and q_heat > 0.0:
        mode = "heating"
    elif q_heat <= 0.0 and q_cool > 0.0:
        mode = "cooling"
    else:
...
def evaluate_controls(bldg: BuildingSpec, cfg: HVACConfig, derived: Dict[str, float], T_mean: float, RH_mean: float, GHI_mean: float, occ: float, year_frac: float, doy: int, rf: float, dust: float, T_sp: float, af: float) -> Dict[str, float]:
    ...
    P_hvac = loads["Q_HVAC_kw"] / max(current_cop, 0.8)
    P_fan = ((derived["Q_air_nom_m3h"] * af / 3600.0 * dp_next / max(cfg.FAN_EFF, 1e-6)) / 1000.0) if getattr(cfg, "USE_HVAC_FANS", True) else 0.0
    P_tot = P_hvac + P_fan
    E_day = P_tot * 24.0
    co2 = E_day * cfg.CO2_FACTOR
...
def simulate_combo(
    ...
    for d in range(cfg.years * cfg.days_per_year):
        ...
        if degradation_model == "physics":
            ...
            P_hvac = loads["Q_HVAC_kw"] / max(current_cop, 0.8)
            P_fan = ((derived["Q_air_nom_m3h"] * af / 3600.0 * dp / max(cfg.FAN_EFF, 1e-6)) / 1000.0) if getattr(cfg, "USE_HVAC_FANS", True) else 0.0
            P_tot = P_hvac + P_fan
            E_day = P_tot * 24.0
            co2 = E_day * cfg.CO2_FACTOR
...
def run_scenario_model(
    ...
    elif axis_mode == "baseline_scenario":
        combos = []
    ...
    if include_baseline_layer:
        ...
        if include_baseline_as_scenario:
            baseline_daily_as_case = baseline_daily_df.copy()
            baseline_annual_as_case = baseline_annual_df.copy()
            baseline_summary_as_case = baseline_summary_df.copy()
            for df_case in [baseline_daily_as_case, baseline_annual_as_case, baseline_summary_as_case]:
                if "strategy" in df_case.columns:
                    df_case["strategy"] = "BASELINE"
                if "scenario_combo_3axis" in df_case.columns:
                    df_case["scenario_combo_3axis"] = "BASELINE_SCENARIO"
            if daily_df.empty:
                daily_df = baseline_daily_as_case
            else:
                daily_df = pd.concat([daily_df, baseline_daily_as_case], ignore_index=True)
            if annual_df.empty:
                annual_df = baseline_annual_as_case
            else:
                annual_df = pd.concat([annual_df, baseline_annual_as_case], ignore_index=True)
            if summary_df.empty:
                summary_df = baseline_summary_as_case
            else:
                summary_df = pd.concat([summary_df, baseline_summary_as_case], ignore_index=True)
